...
import type { TalkPrompt } from "@/lib/content/personal-talk";

type Phase = "loading" | "ready" | "recording" | "scoring" | "done" | "error";

export function PersonalTalkRunner({ kid }: { kid: { id: string; name: string; color_hex: string } }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const r = await fetch("/api/personal-talk/generate", {
        method: "POST",
  ...
  async function submitAudio(blob: Blob) {
    ...
    try {
      ...
      // Reutiliza la evaluación de voz del Diario Hablado (transcribe + puntúa).
      ...
      await fetch("/api/xp/save", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          kidId: kid.id, lessonType: "personal_talk", worldKey: "personal",
          topic: prompt?.prompt_en ?? "Háblame de ti", skill: "speaking", scorePct: avg, xpGained: xp,
  ...
  if (phase === "loading") return <Center><div className="text-4xl mb-3 animate-pulse">🗣️</div><p>Lumi está pensando una pregunta…</p></Center>;
  ...
  if (phase === "done" && evaluation) {
    return (
      <main className="min-h-dvh px-5 py-12 max-w-xl mx-auto relative z-10">
        <h2 className="text-xl font-extrabold mb-1">Háblame de ti</h2>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <Link href={`/personal-talk?kid=${kid.id}`} className="flex-1"><NeonButton variant="primary" className="w-full">Otra pregunta</NeonButton></Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-40 glass-strong px-5 py-3 flex justify-between items-center">
        ...
        <div className="text-xs font-bold text-neon-cyan">🗣️ Háblame de ti</div>
      </header>
      <main className="pt-24 pb-32 px-5 max-w-xl mx-auto relative z-10">
        {/* Lumi hace la pregunta personalizada */}
        <div className="flex flex-col items-center mb-4">
          <LumiCharacter mood="greet" size={120} />
        </div>
        <GlassCard strong glowColor="cyan" className="p-6 mb-4 text-center">
          <div className="text-xs uppercase tracking-widest text-ink-dim mb-2">Lumi te pregunta</div>
        ...
        <div className="text-center">
          {phase === "recording" ? (
            <>
              ...
              <p className="text-xs text-ink-dim mt-3">Hablando en inglés sobre ti…</p>
            </>
          ) : (
            <>
